...
// Imprimir Dashboard
window.imprimirDashboard = async function() {
    const user = JSON.parse(localStorage.getItem('user'));
    ...
    // Buscar dia que mais vendeu
    let melhorDia = '-';
    let melhorDiaReceita = '';
    try {
        const respDias = await fetchAPI('/dashboard/vendas-periodo?dias=30');
        const diasData = await respDias.json();
        if (diasData && diasData.length > 0) {
            const melhor = diasData.reduce((a, b) => parseFloat(a.receita) > parseFloat(b.receita) ? a : b);
            melhorDia = new Date(melhor.data).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' });
            melhorDiaReceita = formatarMoeda(parseFloat(melhor.receita));
        }
    } catch(e) {}

    // Buscar produto mais vendido
    let melhorProduto = '-';
    let melhorProdutoQtd = '';
    try {
        const respProd = await fetchAPI('/dashboard/produtos-mais-vendidos?limite=1');
        const prodData = await respProd.json();
        if (prodData && prodData.length > 0) {
            melhorProduto = prodData[0].nome;
            melhorProdutoQtd = `${prodData[0].totalVendido} unidades vendidas`;
        }
    } catch(e) {}
    ...
    printWindow.document.write(`
        ...
        <body>
            ...
            <div class="kpis">
                ...
                <div class="kpi-card">
                    <div class="kpi-label">Dia que Mais Vendeu (30 dias)</div>
                    <div class="kpi-value">${melhorDia}</div>
                    ${melhorDiaReceita ? `<div class="kpi-sub">${melhorDiaReceita}</div>` : ''}
                </div>
                <div class="kpi-card">
                    <div class="kpi-label">Produto Mais Vendido</div>
                    <div class="kpi-value" style="font-size: 20px;">${melhorProduto}</div>
                    ${melhorProdutoQtd ? `<div class="kpi-sub">${melhorProdutoQtd}</div>` : ''}
                </div>
            </div>
